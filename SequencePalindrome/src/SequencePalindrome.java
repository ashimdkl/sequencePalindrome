...
public final class SequencePalindrome {
    ...
    private static boolean isPalindrome(Sequence<Integer> s) {
        ...
        // temporary number so we can store and compare if it is same.
        Sequence<Integer> backwards = new Sequence1L<>();

        //  the reversed sequence using a for loop
        for (int i = length - 1; i >= 0; i--) {
            int element = s.remove(i);
            backwards.add(backwards.length(), element);
            s.add(i, element);
        }

        // Compare the original sequence with the reversed sequence
        boolean isPalindrome = s.equals(backwards);

        // Restore the original sequence
        for (int i = 0; i < length; i++) {
            int element = s.remove(0); // Remove elements from the front
            s.add(s.length(), element); // Add elements back to the original sequence
        }
